...
import { FiPlusCircle, FiTrash2, FiEdit, FiFolder, FiBookOpen } from 'react-icons/fi';

const AdminPage = () => {
  const [projects, setProjects] = useState<{ _id: string; title: string; description: string; }[]>([]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-12">
          ...
          <div className="p-6">
            {activeTab === 'projects' && (
              ...
              >
                {projects.length === 0 ? (
                  <p className="text-gray-500 col-span-full text-center py-12">No projects found. Add your first project!</p>
                ) : (
            ...
            {activeTab === 'blogs' && (
              ...
              >
                {blogs.length === 0 ? (
                  <p className="text-gray-500 col-span-full text-center py-12">No blogs found. Add your first blog post!</p>
                ) : (
                  blogs.map((blog) => (
                    <motion.div
                      key={blog._id}
      ...
      {showProjectModal && <ProjectModal onClose={() => setShowProjectModal(false)} onProjectAdded={fetchAndSetProjects} />}
      {showBlogModal && <BlogModal onClose={() => setShowBlogModal(false)} onBlogAdded={fetchAndSetBlogs} />}
    </div>
  );
};
